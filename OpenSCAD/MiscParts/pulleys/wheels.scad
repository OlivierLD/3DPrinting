...
module torus(ringDiam, torusDiam) { 
	rotate_extrude(convexity = ringDiam, $fn = 200) {
		translate([ringDiam / 2, 0, 0]) {
			circle(r = torusDiam / 2, $fn = 200);
		}
	}
} 
...
module wheel(ringDiam, torusDiam, axisDiam, wheelThickness) {  
  ...
  difference() {
    cylinder(h=wheelThickness, r=(ringDiam / 2), center=true, $fn=200); // The wheel of the Pulley
    cylinder(h=(wheelThickness * 1.2), r=(axisDiam / 2), center=true, $fn=200);  // Axis
...
*/
WHEEL_TICKNESS = 25; // Used below to override

// Rea Regis
ringDiam =  75;   // RING_DIAM;  // 30;
torusDiam = 18;   // TORUS_DIAM; //  8;  // aka line diam
axisDiam =  12.55; // 12.54, actually // AXIS_DIAM;  //  6;
