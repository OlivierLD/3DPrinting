...
module footedBase(cylHeight, 
	...
	difference() {
		union() {
			...
			if (!feetDown) {
				echo("+------------------------------------------------------------------------");
				echo(">>> WARNING!!: module footedBase => feet orientation facing up!!");
				echo("+------------------------------------------------------------------------");
			}
...
module mainStand(totalStandWidth, 
	...
	axisHeight = (fixingFootSize / 2) + (fixingFootSize / 4); // Tweak the 0.25 value.
	...
	difference() {
		union() {
			cube(size=[bracketWidth * plateBracketRatio, 
								 ...
								 topThickness], 
					 center=true);
			translate([0, 0, -(topThickness * 0.9)]) {
				for (x = [0, 1]) {
					for (y = [0, 1]) {
						translate([x == 0 ? -xPegOffset : xPegOffset, y == 0 ? -yPegOffset : yPegOffset, 0]) {
							cylinder(h=(topThickness * 1.0), d=pegDiam, $fn=50);
						}
					}
				}
			}
...
module panelBracket(mainAxisDiam,
  ...
  heightOutAll = sizeAboveAxis + sizeBelowAxis;
	...
	widthOffset = (widthOutAll / 2) - (1 * thickness) - (footHeightLength / 2); 
	lengthOffset = (plateWidth / 4);
			...
			color("green") {
				difference() {
					oneBracketSide(mainAxisDiam,
												 ...
												 plateWidth,
												 bottomCylinderDiam);
					if (withFixingFeet) { // Drill the holes
						for (y = [0, 1]) {
							translate([0, // -(widthOffset + thickness), 
												 (y == 0 ? lengthOffset : -lengthOffset), 
												 drillingVerticalOffset]) {
								rotate([90, 0, 90]) {
									cylinder(d=screwDiam, h=screwLen, $fn=50, center=true);
								}
							}
  ...
  // left
	if (printOption == ALL_PARTS ||	printOption == LEFT_BRACKET_ONLY) {										
		translate([- ((widthOutAll / 2) - (thickness / 2)), 0, 0]) {
	...
	// top
	if (printOption == ALL_PARTS ||	printOption == TOP_BRACKET_ONLY) {										
		translate([0, 0, (heightOutAll / 2) - (thickness/ 2)]) {
			difference() {
				color("cyan") {
					cube(size=[widthOutAll - ((withFixingFeet ? 2 : 0) * thickness), plateWidth, thickness], center=true);
				}
			...
			if (withFixingFeet) {
				difference() {
					union() {
						for (x = [0, 1]) {
							for (y = [0, 1]) {
								translate([(x == 0 ? -widthOffset : widthOffset), 
													 (y == 0 ? -lengthOffset : lengthOffset), 
													 -(((footHeightLength + thickness) / 2) - 1)]) {
									rotate([90, 180, (x == 0 ? 90 : -90)]) {
										fixingFoot(footHeightLength, footWidth, screwDiam, wallMinThickness, hexBolts);
									}
								}
							}
					...
					hollowDiam = 20;
					translate([0, 0, -thickness - 2]) {
						cylinder(d=hollowDiam, h=hollowLength, center=true, $fn=100);
					}
				}
			}
		}
...
module bevelGearPair(gear_teeth = 60,
										 pinion_teeth = 20,
										 base_thickness = 10,
	...
	if (!with_gear && !with_pinion) {
		...
		difference() {
			...
			if (with_gear) {
				union() {
					// Vertical axis
					cylinder(h=base_thickness * 2.1, d=big_axis_diam, center=true, $fn=50);
...
module wormGearWheelSupport(support_height = 100,
													  big_base_diam = 60, 
														small_base_diam = 40,
													  vert_axis_diam = 5,
													  screw_circle_radius = 8,
	...
	difference() {
		...
		translate([0, 0, -(support_height / 2)]) {		
			cylinder(h=support_height, d1=big_base_diam, d2=small_base_diam, center=true, $fn=100); 
		}

		// Drill vertical axis
		cylinder(h=support_height * 2.1, d=vert_axis_diam, center=true, $fn=50);
		...
		translate([0, 0, -(support_height - fromTop)]) {
			for (angle=[0, 120, 240]) {
				rotate([0, 0, angle]) {
					translate([vert_axis_diam - screwHead + (holeLength / 2), 0, 0]) {
					  cube(size=[holeLength, squareSide, squareSide], center=true);
					}
				}
			}
		}

		// Screws, attaching the support to the top.
		screw_length = 20;
		// screw_circle_radius = 8;
		for (angle=[0, 120, 240]) {
			rotate([0, 0, angle]) {
				translate([screw_circle_radius, 0, -(screw_length + support_height - 6)]) {
					// #cylinder(d=screw_diam, h=50, center=true, $fn=40);
					metalScrewHB(diam=screw_diam, length=screw_length, top=8);
				}
			}
		}
	}
...
if (option == GROOVED_CYLINDER) {
  ...
  footedBase(cylHeight, extDiam, torusDiam, intDiam, ballsDiam, fixingFootSize, fixingFootWidth, screwDiam, minWallThickness);
	...
	echo("Empty!");
	echo(str("Cyl Height:", cylHeight));
	echo(str("Motor side:", _motorSide));

	// End of the playground!
} else if (option == CONIC_GEAR_SPUR_SUPPORT) {
	wormGearWheelSupport(support_height = 60,
											 big_base_diam = 40, 
											 small_base_diam = 30,
											 vert_axis_diam = 5,
											 screw_circle_radius = 8,
